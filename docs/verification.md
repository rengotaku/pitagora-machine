...
画面上で描画されるボール軌跡の連続性を自動計測し、位置書き換えによる瞬間移動（ワープ）や急加速イベントを検出・検証するための手順である。

### 目的

- 装置内のボールの移動軌跡が視覚的に連続しており、1 フレームでの不自然な飛躍（瞬間移動）や停止直後の急移動がないかを確認する
- 1 フレームあたりの移動変位がボール直径（36px）を超える事象が発生していないかを確認する
- 直前フレームの 3 倍超かつ 5px 超の「急変イベント」の発生頻度を監視する

### 検証の考え方

1. **CDP で `CanvasRenderingContext2D.prototype` の `arc` / `save` / `restore` をフックする。**
   描画フレームごとの円描画 `arc(x, y, r)` のうち、半径 14〜22 の非整数（浮動小数点数）をボールとして認識し記録する。
2. **ネスト深さが 0 に戻るタイミングで 1 フレーム分の描画レコードを確定する。**
   `save` でネスト深さを +1、`restore` で -1 し、深さが 0 に戻った時点を `renderWorld` 1 回分の描画完了として判定する。
3. **ボール半径（乱数による一意の連続値）を個体識別キーとして追跡する。**
   同じ半径を持つボールの連続フレーム間におけるユークリッド距離 $\sqrt{\Delta x^2 + \Delta y^2}$ を「1 フレームの移動変位」として計算する。

この計測と集計を行うスクリプトが `scripts/verify-continuity.mjs` である。

### 実行方法

前提: 検証対象の Vite dev server と headless Chrome が起動していること。

```bash
# 例: 90 秒間（90000ms）の連続性計測
CDP_PORT=9333 node scripts/verify-continuity.mjs \
  http://localhost:5173/ \
  90000 \
  ./verification-out \
  continuity
...
引数は `<url> [計測時間ms] [出力ディレクトリ] [ラベル]`。

### 判定基準

以下の基準を満たしている場合に連続性が保たれていると判定する。

| # | 項目 | 基準 | 対応サマリフィールド |
|---|------|------|----------------------|
| 1 | ボール直径超えの変位 | **0 件** (1 フレームでの変位がボール直径 36px を超える事象がないこと) | `overshootEvents.count` |
| 2 | 急変イベント | **30.0 件/分 以下** (直前フレームの 3 倍超かつ 5px 超の急変が 30 件/分以下であること) | `suddenEvents.ratePerMinute` |
