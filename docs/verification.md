...
計測・評価を行う際は、以下の現象による誤判定に注意する。

- **ヘッドフル Chrome のバックグラウンド化（ウィンドウ非表示）**:
  ヘッドフルモードで計測中、Chrome ウィンドウが他のウィンドウの陰に隠れると、ブラウザの描画最適化機能により `requestAnimationFrame` (RAF) が 1fps 程度まで抑制される。この状態では 1 フレームあたりの移動変位が極めて小さく集計されるため、実際にはワープが発生していても**数値上は一見合格に見える誤判定**が起きる。計測結果の妥当性を担保するため、`averageFps` が 60 付近を維持していることを必ず確認する。
- **ヘッドレス環境とヘッドフル環境の物理ステップ差**:
  ヘッドレス環境とヘッドフル環境では、タイマー精度や描画周波数の違いにより 1 フレームあたりに進行する Matter.js の物理ステップ数（0/1/2 ステップ）の揺れ方が異なる。ヘッドレス環境ではステップの揺れが起きやすく急変イベント数が過大に算出される傾向があるため、**修正前後の数値比較や条件検証を行う際は必ず同一の動作環境（ヘッドレス同士またはヘッドフル同士）で統一して計測する**。
